...
func (it *OrderValidatorCounterIncrementedIterator) Next() bool {
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(OrderValidatorCounterIncremented)
...
func (it *OrderValidatorCounterIncrementedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *OrderValidatorCounterIncrementedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}
...
func (_OrderValidator *OrderValidatorFilterer) FilterCounterIncremented(opts *bind.FilterOpts, offerer []common.Address) (*OrderValidatorCounterIncrementedIterator, error) {
	...
	logs, sub, err := _OrderValidator.contract.FilterLogs(opts, "CounterIncremented", offererRule)
	if err != nil {
		return nil, err
	}
	return &OrderValidatorCounterIncrementedIterator{contract: _OrderValidator.contract, event: "CounterIncremented", logs: logs, sub: sub}, nil
}
...
func (_OrderValidator *OrderValidatorFilterer) WatchCounterIncremented(opts *bind.WatchOpts, sink chan<- *OrderValidatorCounterIncremented, offerer []common.Address) (event.Subscription, error) {
	...
	logs, sub, err := _OrderValidator.contract.WatchLogs(opts, "CounterIncremented", offererRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				...
				event := new(OrderValidatorCounterIncremented)
				if err := _OrderValidator.contract.UnpackLog(event, "CounterIncremented", log); err != nil {
					return err
				}
				event.Raw = log
